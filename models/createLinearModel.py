...
model_file_name = "linear-model.pkl"
joblib.dump(linear_model, model_file_name)

# Speichern der MSE-Werte in Textdateien
with open('train_mse.txt', 'w') as train_mse_file:
    train_mse_file.write(str(train_mse))

with open('valid_mse.txt', 'w') as valid_mse_file:
...
container_name_model = f"linear-model-data{container_counter_model}"
while True:
    try:
...
container_name_mse = f"linear-mse-data{container_counter_mse}"
while True:
    try:
...
blob_client_train_mse = blob_service_client.get_blob_client(container=container_name_mse, blob='train_mse.txt')
with open('train_mse.txt', "rb") as data:
    blob_client_train_mse.upload_blob(data)

blob_client_valid_mse = blob_service_client.get_blob_client(container=container_name_mse, blob='valid_mse.txt')
with open('valid_mse.txt', "rb") as data:
